...
typedef struct esl_dsqdata_chunk_s {
  ...
  */
  ...
  while (( status = esl_dsqdata_Read(dd, &chu)) == eslOK) {
      ...
      // Iterate over sequences in chunk
    	for (i = 0; i < chu->N; i++) {
          ...
          // keep track of residues at the beginning
              ...
              if(the_sequence[j] != 20 && the_sequence[j] < 27){
                  num_residues++;
              }
      		}
    ...
    printf("%llu %llu %llu %llu \n", pattern_counts[20], pattern_counts[21], pattern_counts[22], pattern_counts[23]);
    printf("%llu %llu %llu %llu \n", pattern_counts[24], pattern_counts[25], pattern_counts[26], pattern_counts[27]);
    printf("%llu \n\n", pattern_counts[28]);
    printf("count_array_size = %d \n", count_array_size);
    printf("Conditioning on %d residues\n", num_past_residues);
    printf("Saw %d sequences, %llu residues\n", num_sequences, num_residues); // program counts end character of sequences as a residue
